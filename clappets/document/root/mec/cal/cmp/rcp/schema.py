...
class docInput(Schema):
    ...
    predict_etaA = fields.Nested(sXfld, required=True)
    etaA = fields.Nested(sXfld)
    ...
    @validates_schema()
    def check_etaA(self, data):
        if ('predict_etaA' not in data):
            return
        if (data['predict_etaA']['_val'] =='Yes'):
            return
        fName = 'etaA'
        vd.xRequired(data,fName,fName)
        value = data[fName]
